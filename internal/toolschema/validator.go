...
// Validator validates tool call parameters against JSON Schema definitions.
...
// "" at the top level.
func (v *Validator) validateObjectAt(
...
func (v *Validator) validateValue(propDef map[string]any, val any, name string) []string {
	var errs []string
...
func (v *Validator) validateArray(propDef map[string]any, val any, name string) []string {
	arr, ok := val.([]any)
	if !ok {
		return nil
	}
...
func (v *Validator) checkType(name string, val any, expectedType string) string {
	switch expectedType {
	case "string":
...
func (v *Validator) inEnum(val any, allowed []any) bool {
