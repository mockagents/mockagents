...
func (s *PostgresStore) GetUserByEmail(ctx context.Context, email string) (*User, error) {
	row := s.db.QueryRowContext(ctx,
		`SELECT id, email, tenant_id, role, created_at FROM users WHERE email = $1`, email)
	return scanUserRow(row)
}

// CreateUser provisions a new SSO user mapped to a tenant + role.
func (s *PostgresStore) CreateUser(ctx context.Context, email, tenantID string, role Role) (*User, error) {
	if email == "" {
	...
	_, err = s.db.ExecContext(ctx,
		`INSERT INTO users (id, email, tenant_id, role, created_at) VALUES ($1, $2, $3, $4, $5)`,
...
func (s *PostgresStore) CreateSession(ctx context.Context, userID, tenantID string, role Role, ttl time.Duration) (string, *Session, error) {
...
func (s *PostgresStore) ResolveSession(ctx context.Context, token string) (*Principal, error) {
	...
	err := s.db.QueryRowContext(ctx,
		`SELECT user_id, tenant_id, role, expires_at FROM sessions WHERE token_hash = $1`, hash,
	...
	if exp, perr := time.Parse(time.RFC3339, expStr); perr != nil || time.Now().After(exp) {
		_, _ = s.db.ExecContext(ctx, `DELETE FROM sessions WHERE token_hash = $1`, hash)
		return nil, ErrInvalidSession
	}
