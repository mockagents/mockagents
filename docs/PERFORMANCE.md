...
| **PERF-02** | OTel HTTP middleware **allocates per request even when tracing is disabled** (the default) | On *every* HTTP request | S |
| **PERF-03** | Auth `Resolve` writes `last_used` (an **fsync, serialized behind one connection**) on every cache miss | Every first/re-warm auth | S |
...
| **PERF-02** | S | `observability/tracing.go:114-128`, wired at `server/server.go` (`observability.HTTPMiddleware(handler)`) | `HTTPMiddleware` allocates a `statusRecorder`, builds 2 `attribute.String` + a variadic slice for `StartSpan`, copies the request (`r.WithContext`), and calls `SetAttributes` — **on every request, even under the default NoOp tracer**. The engine path already gates this via `observability.IsEnabled()` (`engine.go:82`); the HTTP wrapper doesn't. **Change:** in `server.New`, only add the wrapper when `observability.IsEnabled()` (or early-return inside it). Removes one ResponseWriter wrapper + ~3 allocs + a context copy per request when tracing is off (the default). **Guard:** an HTTP-level bench asserting 0 tracing allocs when disabled. |
...
| **PERF-09** | S–M | `server/log_handlers.go` `InteractionCapture` (`maxCaptureBodyBytes = 1<<20`) | Buffers up to **1 MiB of the response body** on the request goroutine for every loggable LLM response, only so `pricing.ExtractUsage` can re-parse the usage block — which the adapter **already computed** before serializing (`openai.go:179-180`, `RequestMeta` mechanism at `:152-155`). **Change:** stamp `prompt_tokens`/`completion_tokens`/`model` onto `RequestMeta` and drop response-body buffering. Removes a whole-body copy per request + the 1 MiB worst-case pin. (SSE is already correctly skipped.) |
...
1. **PERF-02** (S, drop the tracing wrapper when disabled) and **PERF-08** (S,
   pre-lower scenario literals) — smallest, pure wins, no behavior change.
2. **PERF-03** (S, stop the auth path self-inflicting `last_used` fsyncs) — the
   highest-value auth-path fix.
