...
| R14 | **Drift detection**: three-way triangulation (SDK types ↔ live provider ↔ mock), scheduled-only (never on PR/fork — cost and key exposure), per-provider skip on missing creds, cost-capped, report names the adapter file to fix. **In progress:** provider-neutral offline body/header shape, enum, stream-event order, and named error-contract comparison with configured volatile-path exclusions, JSON/Markdown/SARIF/JUnit reports, exact owner-approved expiring exceptions, a seeded critical-drift test, versioned scrubbed Cohere rerank and OpenAI moderation baselines, and schedule-only single-request collectors with pre-persistence scrubbing are shipped; additional credentialed providers remain. Note: existing `tools/driftcheck` is unrelated (checks repo API refs/licenses). | FR-H01–H05 ≡ PMF F1 (convergent) |
