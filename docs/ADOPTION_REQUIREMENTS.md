...
| R15 | **Chaos everywhere with deterministic seeds. In progress:** a provider-neutral fixed-seed decision layer, explicit per-request force/off override, response decision metadata, documented service/operation precedence for search/rerank/moderation and VectorMock (Qdrant, Pinecone, Chroma), MCP HTTP and stdio (bounded latency/timeouts, disconnects/resets, configurable HTTP statuses with protocol-shaped errors, malformed JSON/schema, bounded response truncation, and per-operation/fixture/sequence rate overrides), and A2A JSON-RPC/HTTP with the same scoped controls while preserving legacy always-on fixtures. Journal recording, explicit strict-replay controls, and global scope remain. | FR-I01–I04 |
