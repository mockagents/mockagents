...
| R12 | Search + rerank + moderation mounts (Tavily-shaped, Cohere-shaped; moderation exists — mount it), with the same chaos/fault treatment as everything else. **In progress:** declarative Tavily substring/regex fixtures, safe fallback, and deterministic latency/status/malformed/disconnect/partial faults are mounted; Cohere rerank and OpenAI moderation are mounted; common cross-service chaos controls remain. | FR-F01–F04 |
