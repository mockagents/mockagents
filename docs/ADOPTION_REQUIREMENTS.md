...
| R14 | **Drift detection**: three-way triangulation (SDK types ↔ live provider ↔ mock), scheduled-only (never on PR/fork — cost and key exposure), per-provider skip on missing creds, cost-capped, report names the adapter file to fix. **In progress:** provider-neutral offline shape comparison, JSON/Markdown/SARIF/JUnit reports, a seeded critical-drift test, and a schedule-only scrubbed Cohere baseline are shipped; credentialed collectors, headers/enums/events/errors, versioned baselines, and expiring exceptions remain. Note: existing `tools/driftcheck` is unrelated (checks repo API refs/licenses). | FR-H01–H05 ≡ PMF F1 (convergent) |
