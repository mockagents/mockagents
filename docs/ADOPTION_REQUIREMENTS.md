...
| R16 | **Selective provider expansion. In progress:** Ollama native chat and Amazon Bedrock native `Converse`/`ConverseStream` are engine-backed. Bedrock covers normalized messages/system prompts, images, tool definitions and tool-use responses, usage/stop metadata, AWS-shaped errors, and checksum-valid AWS EventStream frames in provider event order, plus schema validation and a versioned three-way drift baseline. Vertex/OpenRouter remain demand-gated. Each provider added is a permanent drift-surface obligation. | FR-C02 + PMF F5 |
