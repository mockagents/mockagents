...
| R14 | ✅ **Drift detection**: provider-neutral three-way body/header shape, enum, stream-event order, and named error-contract comparison; configured volatile-path exclusions; JSON/Markdown/SARIF/JUnit reports; exact owner-approved expiring exceptions; seeded critical-drift tests; versioned scrubbed Cohere rerank, OpenAI moderation, and Tavily search baselines; and schedule-only, single-request credentialed collectors that skip when credentials are absent and scrub before persistence. Note: existing `tools/driftcheck` is unrelated (checks repo API refs/licenses). | FR-H01–H05 ≡ PMF F1 (convergent) |
| R15 | ✅ **Chaos everywhere with deterministic seeds:** a provider-neutral fixed-seed decision layer, explicit per-request force/off override, response decision metadata, persisted chaos action/source/effective seed/rate in interaction journals, documented service/operation precedence for search/rerank/moderation and VectorMock (Qdrant, Pinecone, Chroma), MCP HTTP and stdio (bounded latency/timeouts, disconnects/resets, configurable HTTP statuses with protocol-shaped errors, malformed JSON/schema, bounded response truncation, and per-operation/fixture/sequence rate overrides), A2A JSON-RPC/HTTP with the same scoped controls while preserving legacy always-on fixtures, explicit strict replay that rejects upstream configuration, never invokes fallback, returns diagnostic `503` misses, and performs no chaos injection, plus `mockagents start` global seed/rate/off defaults inherited by configured agent, search, rerank, moderation, and VectorMock faults beneath every narrower scope. | FR-I01–I04 |
| R16 | ✅ **Selective provider expansion:** Ollama native chat and Amazon Bedrock native `Converse`/`ConverseStream` are engine-backed. Bedrock covers normalized messages/system prompts, images, tool definitions and tool-use responses, usage/stop metadata, AWS-shaped errors, and checksum-valid AWS EventStream frames in provider event order, plus schema validation and a versioned three-way drift baseline. Vertex/OpenRouter remain demand-gated. Each provider added is a permanent drift-surface obligation. | FR-C02 + PMF F5 |
...
| R20 | ✅ **Migration guides and converters:** `mockagents convert aimock` converts the safely representable AIMock JSON fixture subset to a validated Agent YAML file and reports unsupported matchers rather than broadening them. Guides cover migration from AIMock, MSW, ad hoc SDK mocks, and separate Qdrant/Pinecone/Chroma vector doubles, including real-SDK lifecycle, provider-shaped adapters, overrides, deterministic errors/latency, tools, streaming, vector failures, incremental adoption, and boundaries where existing doubles should remain. vcrpy + OpenAI-stored cassette importers are also available. | After Tier 1 lands | FR-K04 + PMF F8 |
