...
| R12 | Search + rerank + moderation mounts (Tavily-shaped, Cohere-shaped; moderation exists — mount it), with the same chaos/fault treatment as everything else. **In progress:** Tavily, Cohere rerank, and OpenAI moderation now share declarative latency/status/malformed/disconnect/partial controls with provider-shaped errors; Tavily date/domain/pagination parity remains. | FR-F01–F04 |
