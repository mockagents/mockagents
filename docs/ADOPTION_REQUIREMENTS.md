...
| R16 | **Selective provider expansion. In progress:** Ollama native `POST /api/chat` supports engine-backed JSON and default NDJSON streaming responses, messages, images, tool calls, usage counts, native errors, schema validation, and a versioned three-way drift baseline. Bedrock (enterprise blocker) remains next; Vertex/OpenRouter stay demand-gated. Each provider added is a permanent drift-surface obligation. | FR-C02 + PMF F5 |
