...
| R14 | **Drift detection**: three-way triangulation (SDK types ↔ live provider ↔ mock), scheduled-only (never on PR/fork — cost and key exposure), per-provider skip on missing creds, cost-capped, report names the adapter file to fix. **In progress:** provider-neutral offline shape comparison, JSON/Markdown reports, a seeded critical-drift test, and a schedule-only scrubbed Cohere baseline are shipped; credentialed collectors, SARIF/JUnit, headers/enums/events/errors, versioned baselines, and expiring exceptions remain. Note: existing `tools/driftcheck` is unrelated (checks repo API refs/licenses). | FR-H01–H05 ≡ PMF F1 (convergent) |
| R15 | **Chaos everywhere with deterministic seeds. In progress:** a provider-neutral fixed-seed decision layer, explicit per-request force/off override, response decision metadata, and documented precedence are shipped for search/rerank/moderation, VectorMock (Qdrant, Pinecone, Chroma), MCP HTTP and stdio (bounded latency/timeouts, protocol-shaped errors, and malformed JSON), and A2A JSON-RPC (bounded latency/timeouts, protocol-shaped errors, and malformed JSON) while preserving legacy always-on fixtures. Broader fault actions, journal recording, explicit strict-replay controls, and global/service/fixture/operation/sequence scopes remain. | FR-I01–I04 |
