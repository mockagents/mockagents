...
| R12 | Search + rerank + moderation mounts (Tavily-shaped, Cohere-shaped; moderation exists — mount it), with the same chaos/fault treatment as everything else. **In progress:** Cohere v2 deterministic rerank is mounted and OpenAI moderation is confirmed on the common registry; declarative Tavily search fixtures and cross-service faults remain. | FR-F01–F04 |
