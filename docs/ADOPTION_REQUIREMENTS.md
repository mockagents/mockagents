...
| R20 | **In progress:** migration guides and converters. `mockagents convert aimock` converts the safely representable AIMock JSON fixture subset to a validated Agent YAML file and reports unsupported matchers rather than broadening them. The MSW and ad hoc mock migration guides cover declarative LLM scenarios, real-SDK lifecycle, overrides, errors, latency, tools, streaming, incremental adoption, and the boundary where local test doubles should remain. vcrpy + OpenAI-stored cassette importers already exist; separate vector-service doubles remain. | After Tier 1 lands | FR-K04 + PMF F8 |
