...
| R16 | **Selective provider expansion. In progress:** Ollama native chat and Amazon Bedrock native `Converse` are engine-backed. Bedrock `POST /model/{modelId}/converse` covers normalized messages/system prompts, images, tool definitions and tool-use responses, usage/stop metadata, AWS-shaped errors, schema validation, and a versioned three-way drift baseline. Bedrock streaming remains the next slice; Vertex/OpenRouter stay demand-gated. Each provider added is a permanent drift-surface obligation. | FR-C02 + PMF F5 |
