...
| R20 | **In progress:** migration guides and converters. `mockagents convert aimock` converts the safely representable AIMock JSON fixture subset to a validated Agent YAML file and reports unsupported matchers rather than broadening them. The MSW migration guide now covers declarative LLM scenarios, per-test lifecycle, overrides, errors, latency, tools, and the boundary where MSW should remain in place. vcrpy + OpenAI-stored cassette importers already exist; ad hoc mocks and separate vector-service doubles remain. | After Tier 1 lands | FR-K04 + PMF F8 |
