...
| R14 | SR-02, DS-07, DS-09 | implemented; deployment gate passed with documented topology limit | `cc87377`, `4c3926f`, `94f7e31`, `22c5737`, `36e57fb` | Helm matrix, authenticated metrics, Prometheus Operator discovery with target health `up`, PVC restart persistence, rolling update, rollback/forward restore, and exact-image homelab regression pass; k3s `local-path` cross-node drain is explicitly unsupported and now warned/documented | use and test shared storage when cross-node rescheduling is a release requirement |
