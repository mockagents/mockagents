#!/usr/bin/env bash
# Multi-tenancy + quota + cost walkthrough.
#
# Prereqs:
#   * MockAgents started in multi-tenant mode (MOCKAGENTS_MULTI_TENANT=1).
#   * The bootstrap PLATFORM key, captured from the server logs on first start:
#       docker compose logs mockagents | grep "Bootstrap admin key"
#     Pass it in:  BOOTSTRAP_KEY=<key> ./scripts/multitenant_walkthrough.sh
#   * jq + python3 on PATH.
#
# Design note: a platform operator can CREATE tenants but, by design, can only
# mint API keys for its OWN tenant (ensureOwnTenant blocks cross-tenant key
# creation; a new tenant's first key is provisioned out of band). So this script
# operates on the bootstrap key's own "default" tenant: it mints an app key,
# sets a tiny quota override, drives traffic, and reads usage + cost.
set -euo pipefail
...
TENANT_ID=$(curl -fsS "${BASE}/api/v1/tenants" "${PLAT[@]}" \
...
TENANT_KEY=$(curl -fsS "${BASE}/api/v1/tenants/${TENANT_ID}/keys" "${PLAT[@]}" "${JSON[@]}" \
  -d '{"name":"acme-app","role":"editor"}' | jq -r '.plaintext // .Plaintext')
...
say "4. Burst 10 requests with the app key — expect 429 (rate) and/or 402 (spend)"
...
say "5. Tenant quota + usage (as the tenant)"
