...
- [ ] **`X-07` Wire `ValidatePipeline` into the server start path** — `X-07` (new, from validator investigation) · effort:S · **highest-value pipeline fix**
  - `start.go:120-125` registers pipelines with no validation, so the thorough cycle/reachability validator (`pipeline_validator.go`) is bypassed by `mockagents start`/`make run`. Call `config.ValidatePipeline` before `Register` and skip-on-failure, exactly like agents at `start.go:94`. Also wire it into `watcher.go` if pipelines become hot-reloadable. **Done when:** a cyclic pipeline YAML is rejected (logged + skipped) at server start, not just by `mockagents validate`.
- [x] **Pipeline graph: detect cycles, don't drop nodes (engine-side hardening)** — `F-PL-003` / `F-PL-004` / `F-PL-005` · effort:M · owner:claude · **DONE 2026-05-31**
  - Rewrote `runGraph` (`pipeline.go`): (1) **F-PL-004** — walk from *every* zero-indegree root in definition order instead of a single start, so multi-root / disconnected DAGs visit all nodes (in an acyclic graph every node is reachable from some root; only `When`-pruned branches stay unvisited, by design). (2) **F-PL-003/F-PL-005** — added `cyclicNodes` (Kahn's algorithm) and return new sentinel `ErrPipelineCycle` *before* executing anything, replacing the old "fall back to `Agents[0]` and let `visited` truncate the walk" silent-partial-success path. Graph construction (skip self-loops + edges with an unknown endpoint when counting indegree) mirrors `config.validatePipelineGraph` so the executor and load-time validator agree on what is cyclic and which nodes are roots — no new validator↔executor mismatch.
  - **Done when:** ✅ `TestPipelineGraphMultiRoot` (4-node, 2-root DAG runs all 4 — regression-verified: fails with 2 when the root loop `break`s after the first), `TestPipelineGraphCycleErrors` (2-cycle) + `TestPipelineGraphCycleWithFeederErrors` (root + downstream cycle) both expect `ErrPipelineCycle` — regression-verified: both fail when the cycle guard is neutered. Existing `TestPipelineGraphWithConditionalEdge` (single root, `When` skip) still green. Full suite green; gofmt/vet clean.
  - **Note:** `F-PL-010` (`When` substring-vs-equality) and `F-PL-006` (`runParallel` partial-result-on-error contract) are **separate open items**, not touched here.
